...
var poolItemRefundInfo = sync.Pool{
	New: func() any {
		return new(ItemRefundInfo)
	},
}

// GetItemRefundInfo() 从对象池中获取ItemRefundInfo
func GetItemRefundInfo() *ItemRefundInfo {
	return poolItemRefundInfo.Get().(*ItemRefundInfo)
}

// ReleaseItemRefundInfo 释放ItemRefundInfo
func ReleaseItemRefundInfo(v *ItemRefundInfo) {
	v.RefundRegulations = v.RefundRegulations[:0]
	v.RefundRegulationsJson = ""
	v.RefundType = 0
	poolItemRefundInfo.Put(v)
}
