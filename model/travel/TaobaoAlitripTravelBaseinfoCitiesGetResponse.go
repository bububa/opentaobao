...
type TaobaoAlitripTravelBaseinfoCitiesGetAPIResponse struct {
	...
	RequestId     string         `json:"request_id,omitempty" xml:"alitrip_travel_baseinfo_cities_get_response>request_id,omitempty"`         // 平台颁发的每次请求访问的唯一标识

    // 地区级联城市列表，返回数据为json数组结构的字符串
    
    IocInfos   string `json:"ioc_infos,omitempty" xml:"