...
// Alias Setter
// sku销售属性别名；如套餐1 需要调整成其他 需要在这里修改
func (r *AlitripTravelProductGereralskuUpdateRequest) SetAlias(_alias []PropertyAliasInfo) error {
    r._alias = _alias
    r.Set("alias", _alias)
    return nil
}
...
// Properties Setter
// 商品属性列表；由类目的属性PID和VID组成，属性的pid调用taobao.itemprops.get取得，属性值的vid用taobao.itempropvalues.get取得vid。如果该类目下面没有属性，可以不用填写。如果有属性，必选属性必填，其他非必选属性可以选择不填写.属性不能超过35对
func (r *AlitripTravelProductGereralskuUpdateRequest) SetProperties(_properties []CatPropInfo) error {
...
// ItemId Setter
// 淘宝商品ID
func (r *AlitripTravelProductGereralskuUpdateRequest) SetItemId(_itemId int64) error {
    r._itemId = _itemId
...
// Price Setter
// Sku的销售价格。精确到2位小数;单位:分。如:20007，表示:200元7分。修改后的sku价格要保证商品的价格在所有sku价格所形成的价格区间内（例如：商品价格为6元，sku价格有5元、10元两种，如果要修改5元sku的价格，那么修改的范围只能是0-6元之间；如果要修改10元的sku，那么修改的范围只能是6到无穷大的区间中）
func (r *AlitripTravelProductGereralskuUpdateRequest) SetPrice(_price int64) error {
    r._price = _price
    r.Set("price", _price)
    return nil
}
...
// Quantity Setter
// Sku的库存数量。sku的总数量应该小于等于商品总数量(Item的NUM)，sku数量变化后item的总数量也会随着变化。取值范围:大于等于零的整数
func (r *AlitripTravelProductGereralskuUpdateRequest) SetQuantity(_quantity int64) error {
...
// OuterId Setter
// 商家编码
func (r *AlitripTravelProductGereralskuUpdateRequest) SetOuterId(_outerId string) error {
    r._outerId = _outerId
...
// DateList Setter
// SKU的销售价格库存，日历商品使用
func (r *AlitripTravelProductGereralskuUpdateRequest) SetDateList(_dateList []DateInventoryAndPrice) error {
