...
type GroupTourPackageInfo struct {
    ...
    TripElementList  struct {
        GroupTourTripElement  []GroupTourTripElement `json:"group_tour_trip_element,omitempty"`
    } `json:"trip_element_list,omitempty"`
    

    // 返程交通，交通工具的类型必须与商品上的去程交通一致
    
    BackTrafficInfoList  struct {
        GroupTourTrafficInfo  []GroupTourTrafficInfo `json:"group_tour_traffic_info,omitempty"`
    ...
    SelfExplanation  struct {
        String  []string `json:"string,omitempty"`
    } `json:"self_explanation,omitempty"`
