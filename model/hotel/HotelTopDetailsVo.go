...
type HotelTopDetailsVo struct {
    ...
    Labels  struct {
        HotelLabelDto  []HotelLabelDto `json:"hotel_label_dto,omitempty"`
    } `json:"labels,omitempty"`
    

    // 套餐商品
    
    PackRoomTypes  struct {
        PackageRate  []PackageRate `json:"package_rate,omitempty"`
    } `json:"pack_room_types,omitempty"`
    

    // 房型信息
    
    RoomTypes  struct {
        RoomTypeInfo  []RoomTypeInfo `json:"room_type_info,omitempty"`
    } `json:"room_types,omitempty"`
