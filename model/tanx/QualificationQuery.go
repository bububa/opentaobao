...
type QualificationQuery struct {
    ...
    // 生效时间左区间
    StartTimeBegin   string `json:"start_time_begin,omitempty"`

    // 排序字段
    OrderBy   string `json:"order_by,omitempty"`

    // 失效时间左区间
    EndTimeEnd   string `json:"end_time_end,omitempty"`

    // 生效时间右区间
    StartTimeEnd   string `json:"start_time_end,omitempty"`

    // 创建时间右区间
    CreateTimeEnd   string `json:"create_time_end,omitempty"`

    // 审核时间左区间
    AuditTimeBegin   string `json:"audit_time_begin,omitempty"`

    // 资质生效状态(该状态值是根据不能在新增资质时设置)-1=已过期，0=待生效，1=生效中，2=即将过期
    EffectiveStatus   int64 `json:"effective_status,omitempty"`

    // 审核时间右区间
    AuditTimeEnd   string `json:"audit_time_end,omitempty"`

    // 失效时间右区间
    EndTimeBegin   string `json:"end_time_begin,omitempty"`
    ...
    // 正序还是倒序 1是正,0是倒
    Asc   int64 `json:"asc,omitempty"`

    // tanx系统广告主表userId，查询时和user_names选其一
    UserIds   []Number `json:"user_ids,omitempty"`
    ...
    // 查询时包含的资质元素id列表
    ElementIds   []Number `json:"element_ids,omitempty"`
