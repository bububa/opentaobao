...
type OnsiteTradeQueryResponse struct {
    ...
    BuyerTotalFundFee   int64 `json:"buyer_total_fund_fee,omitempty"`
    

    // 消费者优惠总额。消费者支付的营销渠道的总金额，单位为人民币（分）。一般来讲，是通过券／权益抵扣的总金额。消费者付款成功后，才返回此值
    
    BuyerTotalPromotionFee   int64 `json:"buyer_total_promotion_fee,omitempty"`
    

    // 商户实收总额。商户在本次交易中实收的总金额。单位：分
    
    StoreTotalReceivedFee   int64 `json:"store_total_received_fee,omitempty"`
    

    // 商户总营销成本。商户在本次交易中的营销总成本，单位为人民币（分）。
    
    StoreTotalMarketingFee   int64 `json:"store_total_marketing_fee,omitempty"`
    

    // 商户的实收资金渠道明细信息列表。
    
    FundBillList  struct {
        FundBill  []FundBill `json:"fund_bill,omitempty"`
