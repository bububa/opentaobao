...
type OnsiteTradePayRequest struct {
    ...
    AllowablePayChannels   string `json:"allowable_pay_channels,omitempty"`
    

    // 是否自动完成消费者对订单的确认。如果是，则无需消费者确认环节，直接进入待付款。取值：Y、N。缺省值为N
    
    BuyerAutoConfirm   string `json:"buyer_auto_confirm,omitempty"`
    ...
    StoreAlipayAccount   string `json:"store_alipay_account,omitempty"`
    

    // 场景流水号，会员识别时生成。
    
    SceneNo   string `json:"scene_no,omitempty"`
...
type OnsiteTradePayRequest struct {
    ...
    // 商品明细列表。订单包含的商品列表信息
    GoodsDetailList   []GoodsDetail `json:"goods_detail_list,omitempty"`
