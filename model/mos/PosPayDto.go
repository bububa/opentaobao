...
type PosPayDto struct {
    ...
    // 第三方订单号（如支付宝）
    ThirdTradeNo   string `json:"third_trade_no,omitempty"`

    // 原订单号（退货用）
    OriginalTradeNo   string `json:"original_trade_no,omitempty"`

    // 原外部订单号（退货用）
    OriginalOutTradeNo   string `json:"original_out_trade_no,omitempty"`

    // 消费者昵称
    CustomerNickname   string `json:"customer_nickname,omitempty"`
    ...
    // 终端类型（销售渠道）
    SaleChannel   int64 `json:"sale_channel,omitempty"`
