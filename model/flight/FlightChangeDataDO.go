...
type FlightChangeDataDO struct {
    ...
    // 原航变日期 ,格式:yyyy-MM-dd
    OldDepTimeStr   string `json:"old_dep_time_str,omitempty"`

    // 航变类型,1:航班取消 2:航班变更
    FlightChangeType   int64 `json:"flight_change_type,omitempty"`
    ...
    NewDepTimeStr   string `json:"new_dep_time_str,omitempty"`

    // 航班最新出发机场三字码, 字符长度3；仅当flightChangeType=2，该参数必填
    NewDepAirport   string `json:"new_dep_airport,omitempty"`

    // 原出发机场三字代码, 字符长度3
    OldDepAirport   string `json:"old_dep_airport,omitempty"`

    // 变更到的最新航班号,仅当flightChangeType=2,该参数必填
    NewFltNum   string `json:"new_flt_num,omitempty"`
    ...
    // 最新到达时间
    NewArrTimeStr   string `json:"new_arr_time_str,omitempty"`

    // 航班最新到达机场三字码, 字符长度3；仅当flightChangeType=2，该参数必填
    NewArrAirport   string `json:"new_arr_airport,omitempty"`
