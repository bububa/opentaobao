...
type TvmCreateOrderRq struct {
    ...
    Passengers  struct {
        TvmPassengerVo  []TvmPassengerVo `json:"tvm_passenger_vo,omitempty"`
    ...
    TvmBusLineInfo  *struct {
        TvmBusLineInfo  *TvmBusLineInfo `json:"tvm_bus_line_info,omitempty"`
    } `json:"tvm_bus_line_info,omitempty"`
    

    // true实名（传身份证）,必须传true后续进行程序强制校验
    
    RealName   bool `json:"real_name,omitempty"`
    

    // 分润账户明细列表，是个数组，有几个分账，写几个。 注意 只有需要分润到多账号才需要填，否则为空。分账总和等于订单总价。
    
    AccountInDetails  struct {
        AccountInDetail  []AccountInDetail `json:"account_in_detail,omitempty"`
    } `json:"account_in_details,omitempty"`
    ...
    TvmOnline   bool `json:"tvm_online,omitempty"`
    

    // 出票超时时间(支付成功后开始计算) 单位:秒，如果商家不设置，平台默认超时时间为60天(自助机小程序订单使用)
    
    IssueTimeout   int64 `json:"issue_timeout,omitempty"`
    

    // 支付超时时间(创建订单成功后开始计算) 单位:秒,到达指定时间后，平台进行关闭订单操作(自助机小程序订单使用)
    
    PayTimeout   int64 `json:"pay_timeout,omitempty"`
    

    // 创建订单截止时间，不传值默认使用出发时间字段 yyyy-mm-dd HH:mm:ss
    
    CreateDeadline   string `json:"create_deadline,omitempty"`
...
type TvmCreateOrderRq struct {
    ...
    // 乘客
    Passengers   []TvmPassengerVo `json:"passengers,omitempty"`
    ...
    // 分润账户明细列表，是个数组，有几个分账，写几个。 注意 只有需要分润到多账号才需要填，否则为空。分账总和等于订单总价。
    AccountInDetails   []AccountInDetail `json:"account_in_details,omitempty"`
