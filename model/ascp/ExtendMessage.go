...
var poolExtendMessage = sync.Pool{
	New: func() any {
		return new(ExtendMessage)
	},
}

// GetExtendMessage() 从对象池中获取ExtendMessage
func GetExtendMessage() *ExtendMessage {
	return poolExtendMessage.Get().(*ExtendMessage)
}

// ReleaseExtendMessage 释放ExtendMessage
func ReleaseExtendMessage(v *ExtendMessage) {
	v.Reason = ""
	v.DetailReason = ""
	poolExtendMessage.Put(v)
}
