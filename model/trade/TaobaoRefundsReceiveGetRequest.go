...
// 初始化TaobaoRefundsReceiveGetRequest对象
...
// Fields Setter
// 需要返回的字段。目前支持有：refund_id, tid, title, buyer_nick, seller_nick, total_fee, status, created, refund_fee, oid, good_status, company_name, sid, payment, reason, desc, has_good_return, modified, order_status,refund_phase
func (r *TaobaoRefundsReceiveGetRequest) SetFields(_fields []string) error {
    r._fields = _fields
...
// Status Setter
// 退款状态，默认查询所有退款状态的数据，除了默认值外每次只能查询一种状态。WAIT_SELLER_AGREE(买家已经申请退款，等待卖家同意) WAIT_BUYER_RETURN_GOODS(卖家已经同意退款，等待买家退货) WAIT_SELLER_CONFIRM_GOODS(买家已经退货，等待卖家确认收货) SELLER_REFUSE_BUYER(卖家拒绝退款) CLOSED(退款关闭) SUCCESS(退款成功)
func (r *TaobaoRefundsReceiveGetRequest) SetStatus(_status string) error {
    r._status = _status
...
// BuyerNick Setter
// 买家昵称
func (r *TaobaoRefundsReceiveGetRequest) SetBuyerNick(_buyerNick string) error {
...
// Type Setter
// 交易类型列表，一次查询多种类型可用半角逗号分隔，默认同时查询guarantee_trade, auto_delivery这两种类型的数据，<a href="http://open.taobao.com/doc/detail.htm?id=102855" target="_blank">查看可选值</a>
func (r *TaobaoRefundsReceiveGetRequest) SetType(_type string) error {
    r._type = _type
    r.Set("type", _type)
    return nil
}
...
// 查询修改时间开始。格式: yyyy-MM-dd HH:mm:ss
func (r *TaobaoRefundsReceiveGetRequest) SetStartModified(_startModified string) error {
...
// EndModified Setter
// 查询修改时间结束。格式: yyyy-MM-dd HH:mm:ss
func (r *TaobaoRefundsReceiveGetRequest) SetEndModified(_endModified string) error {
...
// PageNo Setter
// 页码。取值范围:大于零的整数; 默认值:1
func (r *TaobaoRefundsReceiveGetRequest) SetPageNo(_pageNo int64) error {
    r._pageNo = _pageNo
...
// PageSize Setter
// 每页条数。取值范围:大于零的整数; 默认值:40;最大值:100
func (r *TaobaoRefundsReceiveGetRequest) SetPageSize(_pageSize int64) error {
...
// UseHasNext Setter
// 是否启用has_next的分页方式，如果指定true,则返回的结果中不包含总记录数，但是会新增一个是否存在下一页的的字段，通过此种方式获取增量退款，接口调用成功率在原有的基础上有所提升。
func (r *TaobaoRefundsReceiveGetRequest) SetUseHasNext(_useHasNext bool) error {
