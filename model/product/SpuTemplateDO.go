...
type SpuTemplateDO struct {
    ...
    KeyProperties   []Number `json:"key_properties,omitempty"`

    // 产品绑定属性，内容为属性ID(PID)的列表,绑定属性肯定在类目上有，对应属性的类目特征，子属性请根据PID到类目上去取
    AffectProperties   []Number `json:"affect_properties,omitempty"`

    // 过滤属性，内容有属性ID(PID)列表，很重要的属性，filter_properties包含的属性，必须填写
    FilterProperties   []Number `json:"filter_properties,omitempty"`
