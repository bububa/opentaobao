...
var poolNormalVisaApplicantOperation = sync.Pool{
	New: func() any {
		return new(NormalVisaApplicantOperation)
	},
}

// GetNormalVisaApplicantOperation() 从对象池中获取NormalVisaApplicantOperation
func GetNormalVisaApplicantOperation() *NormalVisaApplicantOperation {
	return poolNormalVisaApplicantOperation.Get().(*NormalVisaApplicantOperation)
}

// ReleaseNormalVisaApplicantOperation 释放NormalVisaApplicantOperation
func ReleaseNormalVisaApplicantOperation(v *NormalVisaApplicantOperation) {
	v.NeedModifyDocTypes = v.NeedModifyDocTypes[:0]
	v.ApplyId = ""
	v.Remark = ""
	v.ApplicantFormDataJson = ""
	v.EtaInfo = nil
	v.LogisticsInfo = nil
	v.Status = 0
	v.AppointmentInfo = nil
	poolNormalVisaApplicantOperation.Put(v)
}
