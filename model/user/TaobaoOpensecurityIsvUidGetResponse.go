...
type TaobaoOpensecurityIsvUidGetAPIResponse struct {
	...
	RequestId     string         `json:"request_id,omitempty" xml:"opensecurity_isv_uid_get_response>request_id,omitempty"`         // 平台颁发的每次请求访问的唯一标识

    // open security tbUserId for ISV，淘宝账号对ISV级别的唯一open security ID，用于同一个ISV多个Appkey间数据共享。
    
    OpenUidIsv   string `json:"open_uid_isv,omitempty" xml:"