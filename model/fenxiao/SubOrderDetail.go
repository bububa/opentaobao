...
type SubOrderDetail struct {
    ...
    ItemOuterId   string `json:"item_outer_id,omitempty"`
    ...
    Order200Status   string `json:"order_200_status,omitempty"`
    

    // 分销商店铺中宝贝一口价
    
    AuctionPrice   string `json:"auction_price,omitempty"`
    ...
    BillFee   string `json:"bill_fee,omitempty"`
    ...
    TcPreferentialType   string `json:"tc_preferential_type,omitempty"`
    

    // 优惠金额，始终为正数，单位是分，不带小数
    
    TcDiscountFee   int64 `json:"tc_discount_fee,omitempty"`
    

    // 商品的卖出金额调整，金额增加时为正数，金额减少时为负数，单位是分，不带小数
    
    TcAdjustFee   int64 `json:"tc_adjust_fee,omitempty"`
    ...
    TopMemo  *struct {
        TopMemoDto  *TopMemoDto `json:"top_memo_dto,omitempty"`
    } `json:"top_memo,omitempty"`
    

    // 老的SKU属性值。如: 颜色:红色:别名;尺码:L:别名
    
    OldSkuProperties   string `json:"old_sku_properties,omitempty"`
...
type SubOrderDetail struct {
    ...
    // topMemoDTO
    TopMemo   *TopMemoDto `json:"top_memo,omitempty"`
