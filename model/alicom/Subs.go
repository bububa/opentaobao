...
type Subs struct {
    ...
    NeedRecord   bool `json:"need_record,omitempty"`
    

    // 短信通道方式SMS_INTERCEPT(拦截推送阿里)，SMS_NORMAL_SEND(正常现网下发)，SMS_DROP(拦截丢弃)
    
    SmsChannel   string `json:"sms_channel,omitempty"`
    

    // 顺振参数
    
    SequenceCalls  struct {
        SequenceCalls  []SequenceCalls `json:"sequence_calls,omitempty"`
    } `json:"sequence_calls,omitempty"`
    ...
    SequenceTimeout   int64 `json:"sequence_timeout,omitempty"`
    ...
    FastRecord   string `json:"fast_record,omitempty"`
    

    // 是否开启铃音检测 0：不开启 1：开启
    
    RrdsControl   int64 `json:"rrds_control,omitempty"`
