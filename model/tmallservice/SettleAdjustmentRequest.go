...
type SettleAdjustmentRequest struct {
    ...
    PriceFactors  struct {
        SettlementPriceFactor  []SettlementPriceFactor `json:"settlement_price_factor,omitempty"`
    } `json:"price_factors,omitempty"`
...
type SettleAdjustmentRequest struct {
    ...
    // 计价因子，填写规则：1、有计价因子场景：{name:计价因子名称 ,value:数量｝如示例；2、没有计价因子场景：填默认值：｛name:计价因子,value:0｝
    PriceFactors   []SettlementPriceFactor `json:"price_factors,omitempty"`
